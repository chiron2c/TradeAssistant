# --- src/jobs/run.py (FX-aware + config + diagnostic) ---
# Reads config/account.json to set defaults per broker so you don't pass flags each time.
...
def resolve_defaults(broker_key: str, cli_fx: float | None, cli_disable_brent: bool) -> Tuple[float, bool]:
    """
    Choose fx_usd_to_account & disable_brent priority:
      1) CLI overrides
      2) broker-specific config
      3) top-level defaults
      4) hard default (1.0, False)
    """
    cfg = load_config()
    fx = None
    dis = None
...
def recalc_row(
    ...
    broker = choose_broker(row, default_broker)
    ...
    # Compute lots_suggested with account-currency pip value
...
def print_debug(rows: list[Dict[str, Any]], debug_symbol: str):
    ...
    for r in rows:
        ...
        if debug_symbol.upper() in sym:
            hit = True
            print(
                f"  sym={sym:<12} broker={r.get('broker',''):>6} "
...
def main():
    p = argparse.ArgumentParser(description="Normalize allowed_lots; compute lots_suggested using account-currency pip value; debug-print; config aware.")
    ...
    # CLI overrides (optional; config used if these are omitted)
    p.add_argument("--fx-usd-to-account", type=float, default=None, help="Override USD -> Account FX factor (config used if omitted).")
    p.add_argument("--disable-brent", action="store_true", help="Override to disable BRENT for this run.")
    ...
    print(f"[INFO] Broker={broker_key or '(row)'} | fx_usd_to_account={fx_factor} | disable_brent={disable_brent_default}")
    ...
    rows = [recalc_row(r, args.broker, args.mode, args.assume_wti_pip, fx_factor, disable_brent_default) for r in rows]
